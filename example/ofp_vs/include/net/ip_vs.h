...
struct ip_vs_protocol {
	...
	int (*snat_handler) (struct rte_mbuf * skb,
			     struct ip_vs_protocol * pp,
			     struct ip_vs_conn * cp);

	int (*dnat_handler) (struct rte_mbuf * skb,
			     struct ip_vs_protocol * pp,
			     struct ip_vs_conn * cp);

	int (*fnat_in_handler) (struct rte_mbuf * skb,
				struct ip_vs_protocol * pp,
				struct ip_vs_conn * cp);

	int (*fnat_out_handler) (struct rte_mbuf * skb,
				 struct ip_vs_protocol * pp,
				 struct ip_vs_conn * cp);

	int (*csum_check) (int af, struct rte_mbuf * skb,
			   struct ip_vs_protocol * pp);
...
struct mbuf_table {
	unsigned len;
	struct rte_mbuf *m_table[IP_VS_SYNPROXY_SKB_STORE_DEFAULT];
};
...
struct ip_vs_conn {
	...
	/* packet transmitter for different forwarding methods.  If it
	   ...
	   otherwise this must be changed to a rte_mbuf **.
	 */
	int (*packet_xmit) (struct rte_mbuf * skb, struct ip_vs_conn * cp,
			    struct ip_vs_protocol * pp);
	...
	struct mbuf_table ack_skb;	/* ack skb, save in step2 */
	struct rte_mbuf *syn_skb;	/* saved rs syn packet */
...
struct ip_vs_app {
	...
	int (*conn_schedule) (struct rte_mbuf * skb, struct ip_vs_app * app,
			      int *verdict, struct ip_vs_conn ** cpp);

	struct ip_vs_conn *
	    (*conn_in_get) (const struct rte_mbuf * skb, struct ip_vs_app * app,
			    const struct iphdr * iph, unsigned int proto_off,
			    int inverse);

	struct ip_vs_conn *
	    (*conn_out_get) (const struct rte_mbuf * skb, struct ip_vs_app * app,
			     const struct iphdr * iph, unsigned int proto_off,
			     int inverse);
...
extern int ip_vs_app_pkt_out(struct ip_vs_conn *, struct rte_mbuf *skb);
extern int ip_vs_app_pkt_in(struct ip_vs_conn *, struct rte_mbuf *skb);
extern int ip_vs_skb_replace(struct rte_mbuf *skb, gfp_t pri,
...
extern struct ip_vs_conn *ip_vs_schedule(struct ip_vs_service *svc,
					 struct rte_mbuf *skb,
					 int is_synproxy_on);
extern int ip_vs_leave(struct ip_vs_service *svc, struct rte_mbuf *skb,
		       struct ip_vs_protocol *pp);
...
extern void ip_vs_in_stats(struct ip_vs_conn *cp, struct rte_mbuf *skb);
extern void ip_vs_out_stats(struct ip_vs_conn *cp, struct rte_mbuf *skb);
...
extern int ip_vs_null_xmit
    (struct rte_mbuf *skb, struct ip_vs_conn *cp, struct ip_vs_protocol *pp);
...
extern int ip_vs_normal_response_xmit
    (struct rte_mbuf *skb, struct ip_vs_protocol *pp, struct ip_vs_conn *cp,
     int ihl);
extern int ip_vs_fnat_response_xmit(struct rte_mbuf *skb,
				    struct ip_vs_protocol *pp,
				    struct ip_vs_conn *cp, int ihl);
extern int ip_vs_normal_response_icmp_xmit(struct rte_mbuf *skb,
					   struct ip_vs_protocol *pp,
					   struct ip_vs_conn *cp, int offset);
extern int ip_vs_fnat_response_icmp_xmit(struct rte_mbuf *skb,
					 struct ip_vs_protocol *pp,
					 struct ip_vs_conn *cp, int offset);
...
extern int ip_vs_normal_response_xmit_v6
    (struct rte_mbuf *skb, struct ip_vs_protocol *pp, struct ip_vs_conn *cp,
     int ihl);
extern int ip_vs_fnat_response_xmit_v6(struct rte_mbuf *skb,
				       struct ip_vs_protocol *pp,
				       struct ip_vs_conn *cp, int ihl);
extern int ip_vs_normal_response_icmp_xmit_v6(struct rte_mbuf *skb,
					      struct ip_vs_protocol *pp,
					      struct ip_vs_conn *cp,
					      int offset);
extern int ip_vs_fnat_response_icmp_xmit_v6(struct rte_mbuf *skb,
					    struct ip_vs_protocol *pp,
					    struct ip_vs_conn *cp, int offset);
#endif
